...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full backdrop-blur-xl bg-white/80 dark:bg-[#121212]/80 border-b border-gray-200 dark:border-[#27272A] transition-colors duration-300">
          ...
          <nav className="flex items-center bg-gray-100 dark:bg-[#1A1A1A] rounded-lg p-1 shadow-inner border border-gray-200 dark:border-[#333]">
            {['PERFECT 소개', '투어 일정', 'LIVE 중계', '매치 결과', '랭킹'].map((item) => (
              ...
              >
                ...
                {hoveredMenu === item && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-48 bg-white/70 dark:bg-black/70 backdrop-blur-2xl border border-white/40 dark:border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.1)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)] rounded-lg py-3 flex flex-col z-50 animate-in fade-in zoom-in-95 duration-200">
